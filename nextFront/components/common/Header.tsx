    ...
    if (token) {
      try {
        ...
        if (decoded.exp * 1000 < currentTime) {
          ...
          // 🚀 2. 토큰에서 role(권한) 정보를 꺼냅니다.
          // (백엔드 설정에 따라 decoded.role 또는 decoded.auth일 수 있으니 확인 필요)
  ...
  return (
    <>
      ...
      <div className={`drawer-menu ${isOpen ? "open" : ""}`}>
        ...
        <ul>
          <Link href={`/drug`}>
            <li onClick={closeMenu}>일반의약품 검색</li>
          </Link>
          <Link href={`/pill`}>
            <li onClick={closeMenu}>식별검색</li>
          </Link>
          <Link href="/board">
            <li onClick={closeMenu}>게시판</li>
          </Link>
          <Link href="/store/location">
            <li onClick={closeMenu}>약국찾기</li>
          </Link>
          ...
          {isAdmin && (
            <>
              <div
                style={{ margin: "15px 20px", borderTop: "1px solid #eee" }}
              ></div>
              <Link href="/admin">
                <li
                  onClick={closeMenu}
