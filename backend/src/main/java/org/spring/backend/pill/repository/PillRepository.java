...
public interface PillRepository extends JpaRepository<PillEntity, Long> {
    ...
    @Query("SELECT p FROM PillEntity p " +
            "WHERE (:itemName IS NULL OR :itemName = '' OR p.itemName LIKE %:itemName%) " +             "AND (:printFront IS NULL OR :printFront = '' OR " +
    ...
    Page<PillEntity> searchPills(
            @Param("itemName") String itemName,             @Param("printFront") String printFront,
